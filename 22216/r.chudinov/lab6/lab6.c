...
void alarmHandle(int signal) {
	sig_atomic_t countOfElement;
	lseek(file_descriptor, 0, SEEK_SET);
	while(1) {
		countOfElement = read(file_descriptor, bufferForOutput, SIZE_OF_BUFFER);
		if (countOfElement <= 0) {
			break;
		}
		write(1, &bufferForOutput, countOfElement);
	}
...
int main(int argc,char* argv[]) {
	...
	while(1) {
		printf("Enter number of string that you want to see(you have %d seconds)\n", TIMER_TIME);
